"""Mammography evaluation — metrics and plots, baseline vs. Tusi.

Loads the trained model weights and per-epoch history that
``mammography/train.py`` saves to ``mammography/results/``, computes
held-out test-set metrics for each branch, and produces side-by-side
...
from mammography.train import DEFAULT_OUT_DIR, DEFAULT_ROOT_DIR, build_datasets
